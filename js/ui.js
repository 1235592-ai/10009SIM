window.UI = {
    ...
    closeAllPanels: function() {
        try { this.syncPanelsBeforeClose(); } catch(e) { console.error("Sync Error Ignored"); }
    ...
    closeOverlay: function() {
        ...
        else if (App.isPanelOpen) { this.closeAllPanels(); }
    },
    ...
    // 🔥 여기서부터 통째로 유실되었던 렌더링 핵심 함수 12개 전면 복구 완료 🔥
    renderSafetyUI: function() {
        ...
        const safetyObj = Store.state.safety || {}; 
        container.innerHTML = Object.keys(labels).map(key => {
            return `<label style="display:flex; align-items:center; gap:10px; margin-bottom:10px; cursor:pointer; font-size:0.9rem;"><input type="checkbox" ${safetyObj[key] ? 'checked' : ''} onchange="Store.state.safety.${key}=this.checked; Store.saveSettings();"> ${labels[key]}</label>`;
        }).join('');
    },
